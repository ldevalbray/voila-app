#!/usr/bin/env tsx
/**
 * Script pour exécuter la migration 005_create_sprints.sql
 * 
 * Usage: pnpm tsx scripts/run-sprint-migration.ts
 */
...
async function runMigration() {
  console.log('🚀 Démarrage de la migration 005_create_sprints.sql...\n')

  // Lire le fichier de migration
  const migrationPath = join(process.cwd(), 'supabase/migrations/005_create_sprints.sql')
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.log('⚠️  Tentative avec l\'endpoint SQL direct...\n')
      
      // Alternative: utiliser l'API Management
      const mgmtResponse = await fetch(`${supabaseUrl.replace('https://', 'https://api.supabase.com/v1/projects/')}/database/query`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${supabaseServiceRoleKey}`,
    ...
    console.log('   2. Copiez le contenu de supabase/migrations/005_create_sprints.sql')
